...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        ak=body.get("drop_app_key","") or ""
        asc=body.get("drop_app_secret","") or ""
        rf=body.get("drop_refresh","") or ""
        if ak.strip() and asc.strip() and rf.strip():
            results["dropbox"]=test_dropbox(ak.strip(),asc.strip(),rf.strip())
        else:
            results["dropbox"]={"ok":False,"status":"❌ credentials ناقصة","ms":0}
