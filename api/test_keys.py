...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Test Apify
        if body.get("apify_token","").strip():
            results["apify"] = test_apify(body["apify_token"].strip())

        # Test ScraperAPI
        if body.get("scraper_key","").strip():
            results["scraper"] = test_scraper(body["scraper_key"].strip())
